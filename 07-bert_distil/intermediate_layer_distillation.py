...
def model_train(teacher_model, student_model, train_loader, dev_loader, num_epochs, learning_rate, device, save_path):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"\n中间层蒸馏训练 Intermediate Label Distillation Epoch {epoch + 1} / {num_epochs}...")
        # 遍历训练批次
        for batch in tqdm(train_loader, desc=f"Intermediate Label Distillation Epoch {epoch + 1} / {num_epochs}"):
            # 记录当前step开始时间
            ...
            with torch.no_grad():
                # 计算教师模型的概率分布
                teacher_logits,tearcher_hidden = teacher_model(input_ids, attention_mask,return_hidden=True)
                # 获取教师模型的预测值
                teacher_preds = torch.argmax(teacher_logits, dim=1)

            # 获取学生模型的输出logits
            student_logits, student_project_hidden = student_model(input_ids, attention_mask, return_hidden=True)

            # 计算中间层损失（MSELoss）
            # 软标签损失=MSELoss
            inter_loss = F.mse_loss(student_project_hidden, tearcher_hidden)
